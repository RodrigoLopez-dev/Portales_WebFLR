...
{
    $secret = env_value('FINTOC_WEBHOOK_TOKEN', '');

    if ($secret === '') {
        fintoc_log('FINTOC_WEBHOOK_TOKEN no configurado.');
...
$order_id = isset($metadata['order_id']) ? trim($metadata['order_id']) : '';
$origen = isset($metadata['origen']) ? trim($metadata['origen']) : '';
