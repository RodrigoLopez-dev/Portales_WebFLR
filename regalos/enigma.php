...
class Enigma {
  ...
  public static function encode_this($string)
  {
  $string = utf8_encode($string);
  $control = "fundicion"; //defino la llave para encriptar la cadena, cambiarla por la que deseamos usar
  $string = $control.$string.$control; //concateno la llave para encriptar la cadena
  $string = base64_encode($string);//codifico la cadena
  return($string);
  }

  public static function decode_get2($string)
  {
  ...
  $control = "fundicion"; //defino la llave con la que fue encriptada la cadena,, cambiarla por la que deseamos usar
