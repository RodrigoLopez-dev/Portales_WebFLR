...
$.ajax({
    url: "crud/traeListaCoronasVirtuales.php",
    type: "GET",
    async:true,
    dataType: "json"
})
 .done(function( data, textStatus, jqXHR ) {
   ...
   var cabeza = '<table id="tabla_completa" class="table table-striped table-bordered" style="width:100%">'+
      '<thead>'+
       ...
       '<th style="width:15px">Fallecido</th>'+
       '<th style="width:15px">Quien envia</th>'+
      '<th style="width:15px">Quien recibe</th>'+
...
var id = encode_this('id='+data[x]['orden_id']+'&medioPago='+medio_pago);
       ...
       tabla = tabla +
          ...
          '<td> '+data[x]['nombreFallecido']+'</td>'+
          '<td> '+data[x]['quienEnvia']+'<br>'+data[x]['emailEnvia']+'</td>'+
          '<td> '+data[x]['AquienEnvia']+'<br>'+data[x]['emailRecibe']+'</td>'+
          '<td> '+data[x]['estado']+'<br>'+data[x]['fecha']+'</td>'+
          '<td><a target="_blank" href="https://fundacionlasrosas.cl/coronasvirtuales/reenvioCorreo.php?'+id+'" title="REENVIAR" class="btn btn-info">Reenviar</a></td>'+
